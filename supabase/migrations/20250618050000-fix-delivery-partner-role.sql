-- Fix all delivery partner roles and ensure RLS policies use the correct role name

-- 1. Update any user_roles with 'delivery_partner' to 'delivery'
UPDATE public.user_roles SET role = 'delivery' WHERE role = 'delivery_partner';

-- 2. Remove any duplicate roles (if any)
DELETE FROM public.user_roles WHERE role = 'delivery_partner';

-- 3. Re-apply the correct RLS policies for delivery partners
...
-- 4. Update the validation function to use the correct role name
